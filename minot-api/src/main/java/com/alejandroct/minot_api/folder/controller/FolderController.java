...
//    }
    ...
    @PostMapping
    public ResponseEntity<FolderDTO> save(@Valid @RequestBody FolderDTO folderDTO, Principal principal){
        return new ResponseEntity<>(this.folderService.save(folderDTO, principal.getName()), HttpStatus.CREATED);
    }

    @PutMapping("/{id}")
    public ResponseEntity<FolderDTO> update(@RequestBody FolderDTO folderDTO, @PathVariable Long id, Principal principal){
        return ResponseEntity.ok(this.folderService.update(folderDTO, id, principal.getName()));
    }
}
